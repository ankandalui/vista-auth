...
async function prompts(questions) {
  ...
  for (const question of questions) {
    if (question.type === "select") {
      ...
      const answer = await new Promise((resolve) => {
        rl.question(
          "\nEnter numbers (e.g., 1,3,4) or press Enter for defaults: ",
          (input) => {
            if (!input.trim()) {
              resolve(
                question.choices.filter((c) => c.selected).map((c) => c.value)
              );
            } else {
              const indices = input
                .split(",")
              ...
              resolve(
                indices.map((i) => question.choices[i]?.value).filter(Boolean)
              );
            }
          }
        );
