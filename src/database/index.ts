  ...
  return {
    ...
    async createUser(data: Partial<User>) {
      const result = await pool.query(
        ...
        [data.id, data.email, data.name, data.roles, data.permissions, data.metadata]
      );
    ...
    async createSession(userId: string, sessionData: any) {
      await pool.query(
        ...
        [sessionData.sessionId, userId, new Date(sessionData.expiresAt), sessionData]
      );
      return sessionData as Session;
    },
...
export function createFirebaseAdapter(firestore: any): DatabaseAdapter {
  return {
    ...
    async createSession(userId: string, sessionData: any) {
      await firestore.collection('sessions').doc(sessionData.sessionId).set({
        userId,
        expiresAt: new Date(sessionData.expiresAt),
        data: sessionData,
      });
    ...
    async deleteUserSessions(userId: string) {
      const snapshot = await firestore.collection('sessions').where('userId', '==', userId).get();
